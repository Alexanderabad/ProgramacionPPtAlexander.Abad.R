...
void main(void)
{
 ...
 printf("\n X = %d \t Y = %d \t Z [0] = %d \t Z[1] = %d \t Z[3] = %d \t Z[4] = %d",
    ...
    int *IX;     /* XI reresenta un apuntador a un entero. */
    ...
    *IX = 15;
    ...
    X = *IX + 5;   /* X se modifica, ahora vale Z[2] + 5 = 20. recuerda que *IX contiene el valos de Z[2]. */
    *IX = *IX - 5; /* se modifica ahora vale 10. */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d Z[2] = %d \t Z[3] = %d \t Z[4] = %d",
            X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
    ...
    *IX += 1;   /*Z [2] se modifica ahora vale 12. */
    ...
    X = *(IX + 1);   /* X se modifica. el apuntador IX accede temporalmente a Z[3], por lo tanto toma este valor (8). obseva que IX no se reasigna */
    Y = *IX;         /* Y se modifica, toma el valor de Z[2] (12). */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d Z[2] = %d \t Z[3] = %d \t Z[4] = %d",
            X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

    IX = IX + 1;   /* IX se modifica. observa la forma de mover el apuntador. ahora IX apunta al cuarto elemento de (Z[3]). */
    Y = *IX;       /* Y se modifica, ahora vale Z[3] (8). */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d Z[2] = %d \t Z[3] = %d \t Z[4] = %d",
            X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

    IX = IX + 4;   /* IX se modifica. Observa que el apuntador se mueve4 posiciones y cae en una dirección afuera del arreglo. Esto ocasionará unerror. */
